...
public class CategorieCrud {
	...
	URI uri=UriBuilder.fromUri("http://localhost:8084/magasin/").build();
	    ...
	    try {
	    	...
	    	if(rootNode.has("categorie")) {
	    		...
	    		if(categorienode.isArray()) {
		    		...
		    		if (list.isEmpty()) {
		            System.out.println("Aucun produit trouvée.");
		        } else {
...
public void ajouterCategorie(int code, String designation) {
    ...
    try {
        ...
        if (response != null) {
            ...
            if (status == 200) {
                System.out.println("\t\t\t\t\tCategorie ajoutÃ© avec succées !");
            } else {
            	System.out.println("\t\t\t\t\tcette catÃ©gorie est deja exist avec ce code");
            }
            response.close(); 
        }
    } catch (Exception e) {
...
public void updateCategorie(int code, String designation) {
    ...
    try {
        ...
        if (response != null) {
            int status = response.getStatus();
            if (status == 200) {
                System.out.println("\t\t\t\t\tCategorie mis Ã  jour avec succÃ¨s  !");
            } else {
                System.out.println("\t\t\t\t\tErreur lors de la mis Ã  jour du Categorie : " + status);
            }
            response.close(); 
        }
    } catch (Exception e) {
...
public void supprimerCategorie(int code) {
    try {
        ...
        if (response.getStatus() == 200) {
            System.out.println("\t\t\t\t\tCategorie supprimÃ© avec succÃ¨s !");
        } else {
...
public static void displayCategorie(int code, String designation) {
	  System.out.printf("\t│ %-10d │ %-20s │%n", code, designation);
	    System.out.println("\t├────────────┼─────────────────────┤");
}
}
